...
class nnUNetPredictor(object):
    ...
    def _manage_input_and_output_lists(self, list_of_lists_or_source_folder: Union[str, List[List[str]]],
        ...
        # Si la entrada es una carpeta, obtenemos la lista de archivos
        if isinstance(list_of_lists_or_source_folder, str):
            print("🔄 Escaneando carpeta de entrada...")
            with os.scandir(list_of_lists_or_source_folder) as dir_entries:
                list_of_lists_or_source_folder = [
                    [join(list_of_lists_or_source_folder, entry.name)]
                    for entry in dir_entries if entry.is_file() and entry.name.endswith(self.dataset_json['file_ending'])
                ]
    
        print(f"✅ [TIME] Escaneo de archivos de entrada: {time.time() - start_time:.2f} s")
        ...
        # Construcción de nombres de salida
        ...
        # Generar rutas de segmentaciones previas
        ...
        # Si overwrite=False, filtrar archivos existentes más rápido
        if not overwrite and output_filename_truncated is not None:
            print("🔍 Verificando archivos ya existentes...")
            start_time = time.time()
            
            with multiprocessing.Pool(processes=4) as pool:
                file_check_results = pool.map(isfile, [i + self.dataset_json['file_ending'] for i in output_filename_truncated])
    
            if save_probabilities:
                with multiprocessing.Pool(processes=4) as pool:
                    prob_check_results = pool.map(isfile, [i + '.npz' for i in output_filename_truncated])
                file_check_results = [i and j for i, j in zip(file_check_results, prob_check_results)]
    
            not_existing_indices = [i for i, j in enumerate(file_check_results) if not j]
            ...
            print(f"✅ [TIME] Verificación de archivos existentes: {time.time() - start_time:.2f} s")
            print(f'overwrite={overwrite}, {len(not_existing_indices)} casos restantes para predecir.')
    ...
    def predict_from_files(self,
                           ...
                           part_id: int = 0):
        """
        Realiza la predicción en múltiples imágenes, con medición de tiempos optimizada.
        """
        start_time_total = time.time()  # Medición del tiempo total
    
        # Determinar el output_folder
        start_time = time.time()
        if isinstance(output_folder_or_list_of_truncated_output_files, str):
            ...
            output_folder = None
        print(f"✅ [TIME] Determinar output_folder: {time.time() - start_time:.2f} s")
    
        # Obtener listas de entrada y salida
        start_time = time.time()
        list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files = \
            self._manage_input_and_output_lists(list_of_lists_or_source_folder,
                                                ...
                                                save_probabilities)
        print(f"✅ [TIME] Gestión de nombres de archivos: {time.time() - start_time:.2f} s")
    
        if len(list_of_lists_or_source_folder) == 0:
            print("🚫 No hay archivos para predecir.")
            return
        ...
        print(f"✅ [TIME] Carga de datos y preprocesamiento: {time.time() - start_time:.2f} s")
    
        # Llamar a la función de predicción real
        start_time = time.time()
        result = self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export)
        print(f"✅ [TIME] Tiempo total de predicción: {time.time() - start_time_total:.2f} s")
    ...
    def predict_from_data_iterator(self,
                                   ...
                                   num_processes_segmentation_export: int = default_num_processes):
        """
        Predice desde un iterador de datos y exporta los resultados, optimizando transferencia a GPU y memoria.
        """
        ret = []  # Lista para almacenar los archivos exportados
        ...
        for preprocessed in data_iterator:
            ...
            if ofile is not None:
                print(f'\nPredicting and exporting for: {os.path.basename(ofile)}')
            else:
                print('\nPredicting for an image without output file.')
    
            print(f'perform_everything_on_device: {self.perform_everything_on_device}')
    
            # Recuperar datos y mover a GPU si es necesario
            start_time = time.time()
            ...
            if isinstance(data, str):
                ...
                os.remove(delfile)
            print(f"✅ [TIME] Carga en memoria: {time.time() - start_time:.2f} s")
    
            start_time = time.time()
            data = data.to(self.device)
            print(f"✅ [TIME] Transferencia a {self.device}: {time.time() - start_time:.2f} s")
    
            # Realizar la predicción
            start_time = time.time()
            prediction = self.predict_logits_from_preprocessed_data(data).cpu()
            print(f"✅ [TIME] Inferencia: {time.time() - start_time:.2f} s")
    
            # Validar dimensiones y valores de predicción
            print(f'Prediction shape: {prediction.shape}, dtype: {prediction.dtype}')
            print(f'Prediction max: {prediction.max()}, min: {prediction.min()}')
    
            # Exportar resultados si hay un archivo de salida definido
            if ofile is not None:
                start_time = time.time()
                try:
                    ...
                    print(f"✅ [TIME] Exportación: {time.time() - start_time:.2f} s")
                except Exception as e:
                    print(f"❌ Error exportando {os.path.basename(ofile)}: {e}")
                    raise
    
            # Agregar el archivo exportado a los resultados
            ret.append(ofile)
